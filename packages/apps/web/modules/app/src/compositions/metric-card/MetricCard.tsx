...
export function MetricCard({
  ...
  if (layout === "horizontal") {
    return (
      <Card className={cn("rounded-2xl p-5 border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-theme-xs", className)}>
          ...
          <div className="min-w-0 flex-1">
            ...
            {badgeWithComparison ? (
              <div className="flex items-center gap-2 mt-1">
                <span className="text-xs text-gray-500 dark:text-gray-400 font-medium truncate">{title}</span>
                {badgeWithComparison}
              </div>
            ) : (
              <p className="text-xs text-gray-500 dark:text-gray-400 font-medium truncate mt-0.5">{title}</p>
            )}
          </div>
        </div>
      </Card>
  ...
  if (layout === "compact") {
      ...
      return (
        <Card className={cn("rounded-2xl p-5 border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-theme-xs", className)}>
          ...
          <div className="flex items-end justify-between mt-4">
            <p className="text-gray-600 text-xs font-medium dark:text-gray-400">
              {title}
            </p>
            {badgeWithComparison}
          </div>
    ...
    return (
      <Card className={cn("rounded-2xl p-5 border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-theme-xs", className)}>
        <p className="text-gray-500 text-xs font-medium dark:text-gray-400">
          {title}
        </p>
    ...
    <Card className={cn("rounded-2xl p-5 border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-theme-xs", className)}>
      ...
      <div className="mt-4">
        ...
        {comparisonText && !badgeWithComparison && (
          <p className="text-[11px] text-gray-400 mt-1">{comparisonText}</p>
        )}
      </div>
    </Card>
  );
