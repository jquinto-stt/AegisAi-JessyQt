      ...
      <div className="relative w-full max-w-xl bg-white dark:bg-[#121215] shadow-2xl border-l border-zinc-200/80 dark:border-zinc-800/90 flex flex-col h-full z-10 animate-slide-left overflow-hidden">
        ...
        <div className="p-5 sm:p-6 border-b border-zinc-200/80 dark:border-zinc-800/80 bg-zinc-50/50 dark:bg-zinc-900/40 flex items-center justify-between gap-4 flex-none">
          <div className="flex items-center gap-3.5 min-w-0">
            <div className="w-11 h-11 rounded-2xl bg-orange-500/10 dark:bg-orange-500/20 text-[#FF3F1A] flex items-center justify-center font-bold text-sm border border-[#FF3F1A]/30 font-mono shadow-2xs flex-none">
              #{order.turnNumber || "00"}
            </div>
          ...
          <div className="flex items-center gap-2 flex-none">
            ...
            <Button
              ...
              onClick={() => setSelectedOrderId(null)}
              className="w-8 h-8 p-0 rounded-xl text-zinc-400 cursor-pointer"
            >
              <X className="w-4 h-4" />
            </Button>
          </div>
        ...
        {/* View Mode Toggle: Ficha Técnica Cocina vs Detalle General */}
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6">
          ...
          {drawerViewMode === "cocina" ? (
            <div className="space-y-5 animate-fade-in">
              ...
              <div className="p-4 rounded-3xl bg-[#ECECEC] dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-2xl bg-[#FF3F1A] text-white flex items-center justify-center font-bold shadow-xs">
              ...
              {order.notes ? (
                <div className="p-4 rounded-2xl bg-amber-500/10 border-2 border-amber-500/30 text-amber-950 dark:text-amber-100 space-y-1 shadow-2xs">
                  <div className="flex items-center gap-2 font-black text-xs uppercase tracking-wider text-amber-800 dark:text-amber-300">
                    <AlertTriangle className="w-4 h-4 text-amber-600 dark:text-amber-400 flex-none" />
                    <span>Instrucción Crítica / Alérgenos</span>
                  </div>
                  <p className="text-xs font-bold leading-relaxed">{order.notes}</p>
                </div>
              ) : (
                <div className="p-3 rounded-2xl bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200/60 dark:border-zinc-800/60 text-xs text-zinc-500 flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
                  <span>Sin alérgenos ni notas especiales declaradas.</span>
                </div>
              )}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="font-bold text-xs uppercase tracking-wider text-zinc-500 flex items-center gap-1.5">
                    <CheckSquare className="w-4 h-4 text-[#FF3F1A]" />
                    <span>Checklist de Elaboración & Empaque</span>
                  </h4>
                ...
                <div className="border border-zinc-200 dark:border-zinc-800 rounded-3xl overflow-hidden divide-y divide-zinc-100 dark:divide-zinc-800/80 bg-white dark:bg-zinc-900 shadow-2xs">
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 flex items-start justify-between gap-3 cursor-pointer transition-colors ${
                          isDone
                            ? "bg-emerald-500/[0.04] text-zinc-400 dark:text-zinc-500"
                      ...
                      >
                        <div className="flex items-start gap-3 min-w-0">
                          ...
                          <div className="space-y-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span
                                className={`text-sm font-mono font-black px-2 py-0.5 rounded-lg ${
                                  isDone
                            ...
                            {it.option && (
                              <span className="text-[11px] font-bold text-orange-600 dark:text-orange-400 bg-orange-500/10 px-2 py-0.5 rounded-md inline-block">
                                {it.option}
                              </span>
                            )}
                            {it.notes && (
            ...
            <div className="space-y-6 animate-fade-in">
                      ...
                      <div>
                        ...
                        <h4 className="font-bold text-xs text-zinc-950 dark:text-zinc-50 mt-0.5">
                          {activeIncident?.title ||
                            `Retraso en Comanda ${order.id} (+${delayMinutes > 0 ? delayMinutes : 6}m sobre pactado)`}
                        </h4>
                      </div>
                    </div>
              ...
              <div className="bg-zinc-50/70 dark:bg-zinc-900/60 rounded-3xl p-5 border border-zinc-200/80 dark:border-zinc-800 space-y-3.5">
                <h4 className="font-bold text-[11px] uppercase tracking-wider text-zinc-400 flex items-center gap-1.5">
                  <User className="w-3.5 h-3.5" /> Información del Cliente & Cobro
                </h4>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                  ...
                  {order.paymentMethod && (
                    <div>
                      <span className="text-zinc-400 block text-[11px]">Método de Pago</span>
                      <p className="font-bold text-zinc-900 dark:text-white uppercase font-mono mt-0.5">
                        {order.paymentMethod}
                      </p>
                    </div>
                  )}
              ...
              <div className="space-y-3">
                <h4 className="font-bold text-[11px] uppercase tracking-wider text-zinc-400 flex items-center gap-1.5">
                  <Receipt className="w-3.5 h-3.5" /> Detalle de Productos & Factura
                </h4>
        ...
        <div className="p-5 border-t border-zinc-200/80 dark:border-zinc-800 bg-zinc-50/70 dark:bg-zinc-900/60 flex items-center gap-3 flex-none">
          {programados.some(p => p.id === order.id) && (
            <Button
              ...
              className="flex-1 py-3 px-4 rounded-2xl text-xs bg-[#FF3F1A] hover:bg-[#e03715] text-white font-bold cursor-pointer"
            >
          ...
          {!programados.some(p => p.id === order.id) && order.status === "NUEVO" && (
            <>
              ...
              <Button
                ...
                className="py-3 px-4 rounded-2xl border-rose-200 dark:border-rose-800/80 text-rose-600 dark:text-rose-400 hover:bg-rose-50 text-xs cursor-pointer"
              >
                Rechazar
              </Button>
            </>
          ...
          {order.status === "EN_PREPARACION" && (
            <Button
              ...
              className="flex-1 py-3 px-4 rounded-2xl text-xs bg-emerald-600 hover:bg-emerald-500 text-white font-bold cursor-pointer"
            >
          ...
          {!["FINALIZADO", "CANCELADO", "RECHAZADO"].includes(order.status) && (
            <Button
              ...
              className="py-3 px-3.5 rounded-2xl text-zinc-400 hover:text-rose-600 text-xs cursor-pointer"
