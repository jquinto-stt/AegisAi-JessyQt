...
export const PedidosEnVivoView: React.FC<{
  ...
  const handleDropOrder = (orderId: string, targetCol: OrderStatus) => {
    ...
    transitionOrder(
      ...
      `Comanda movida al estado ${targetCol} mediante arrastre en el tablero.`
    );
    ...
    setDragToast(`Comanda ${orderId} trasladada a "${colNames[targetCol]}"`);
  ...
  const columns: Array<{
  ...
  }> = [
    {
      ...
      headerColor: "text-blue-600 dark:text-blue-400 border-blue-500",
    ...
    },
    {
      ...
      headerColor: "text-[#190088] dark:text-indigo-300 border-[#190088]",
      badgeBg: "bg-indigo-100 dark:bg-indigo-950 text-[#190088] dark:text-indigo-300",
      icon: CheckCircle2,
    },
    {
      id: "EN_PREPARACION",
      ...
      headerColor: "text-[#FF3F1A] dark:text-orange-400 border-[#FF3F1A]",
      badgeBg: "bg-orange-100 dark:bg-orange-950 text-[#FF3F1A] dark:text-orange-300",
      icon: ChefHat,
    },
    {
      id: "LISTO",
      ...
      headerColor: "text-emerald-600 dark:text-emerald-400 border-emerald-500",
    ...
    },
    {
      ...
      headerColor: "text-slate-600 dark:text-slate-400 border-slate-400",
  ...
  const getChannelBadge = (channel: OrderChannel) => {
    switch (channel) {
      case "whatsapp":
        return { label: "WhatsApp IA", bg: "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/80 dark:text-emerald-300 border-emerald-200" };
      case "web":
        return { label: "Web Directo", bg: "bg-blue-50 text-blue-700 dark:bg-blue-950/80 dark:text-blue-300 border-blue-200" };
      case "presencial":
        return { label: "Mostrador / Salón", bg: "bg-orange-50 text-orange-700 dark:bg-orange-950/80 dark:text-orange-300 border-orange-200" };
      case "telefono":
        return { label: "Teléfono", bg: "bg-purple-50 text-purple-700 dark:bg-purple-950/80 dark:text-purple-300 border-purple-200" };
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <NectoBanner
        ...
        title="Tablero de Comandas"
        description="Flujo operativo visual tipo Kanban en tiempo real: desde la recepción multicanal hasta la entrega al cliente."
        actionNode={
          <div className="flex items-center gap-2 flex-wrap">
            {/* View Mode Toggle */}
            <div className="flex bg-slate-100 dark:bg-gray-800 p-1 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-xs">
              <button
              ...
              >
                <Kanban className="w-3.5 h-3.5" /> Tablero Kanban
              </button>
              <button
            ...
            <button
              ...
              className="py-2.5 px-4 rounded-2xl bg-[#FF3F1A] hover:bg-orange-600 text-white text-xs font-black flex items-center gap-2 shadow-xs transition-all cursor-pointer active:scale-95 flex-none"
            >
              <Plus className="w-4 h-4" /> Nueva Comanda
            </button>
          </div>
        }
      />

      {/* Toolbar: Search & Multichannel Filters */}
      <div className="bg-white dark:bg-[#2C2D31] rounded-3xl p-4 border border-slate-200/90 dark:border-[#374151] shadow-xs flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="relative flex-1 min-w-[260px] max-w-md">
          ...
          <input
            ref={searchInputRef}
            type="text"
            placeholder="Buscar por #turno, cliente, teléfono, empanada..."
            value={searchQuery}
            onChange={e => setSearchQuery(e.target.value)}
            className="w-full pl-9 pr-20 py-2.5 text-xs bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-2xl text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-orange-500/30"
          />
        ...
        {/* Channel Filters */}
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar max-w-full flex-nowrap sm:flex-wrap py-1 text-xs font-bold">
          <span className="text-gray-400 text-[11px] uppercase mr-1 flex-none">Canal:</span>
          {(["TODOS", "whatsapp", "web", "presencial", "telefono"] as const).map(ch => (
            <button
              key={ch}
              onClick={() => setChannelFilter(ch)}
              className={`py-1.5 px-3 rounded-xl text-xs font-extrabold transition-all cursor-pointer flex-none ${
                channelFilter === ch
                  ? "bg-[#190088] text-white shadow-xs"
                  : "bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-gray-600 dark:text-gray-300"
              }`}
            >
              {ch === "TODOS"
                ? "Todos"
        ...
        <div className="flex items-center gap-1.5 text-xs font-bold">
          <button
            ...
            className={`py-1.5 px-3 rounded-xl text-xs font-extrabold transition-all cursor-pointer flex items-center gap-1.5 border ${
              urgencyFilter === "RETRASADO"
                ? "bg-red-500 text-white border-red-500 shadow-xs ring-2 ring-red-500/20"
                : "bg-slate-50 dark:bg-gray-800 border-slate-200 dark:border-gray-700 text-red-600 dark:text-red-400"
            }`}
          >
      ...
      {/* Cross-Module Operational Live Bridge */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 text-xs font-bold">
        {/* Bridge to KDS Cocina */}
        <div
          onClick={() => onNavigateOpTab?.("preparacion")}
          className="bg-orange-50/80 hover:bg-orange-100/90 dark:bg-orange-950/40 dark:hover:bg-orange-900/40 border border-orange-200 dark:border-orange-900/60 rounded-2xl p-3.5 flex items-center justify-between transition-all cursor-pointer shadow-xs group"
        >
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-white dark:bg-gray-800 text-[#FF3F1A] flex items-center justify-center shadow-xs">
              <ChefHat className="w-4.5 h-4.5" />
            </div>
            <div>
              <p className="font-black text-gray-900 dark:text-gray-100">
                KDS Cocina en Vivo: <span className="text-[#FF3F1A]">{inKitchenCount} en fogón</span>
              </p>
              <p className="text-[11px] text-gray-500 dark:text-gray-400">
                Checklist de horneado y cronómetro por tanda en tiempo real.
              </p>
            </div>
          </div>
          <span className="text-[#FF3F1A] text-xs font-black flex items-center gap-1 group-hover:translate-x-1 transition-transform">
            Ver Pantalla KDS <ArrowRight className="w-3.5 h-3.5" />
          </span>
        </div>

        {/* Bridge to Scheduled Orders */}
        <div
          onClick={() => onNavigateOpTab?.("programados")}
          className="bg-indigo-50/80 hover:bg-indigo-100/90 dark:bg-indigo-950/40 dark:hover:bg-indigo-900/40 border border-indigo-200 dark:border-indigo-900/60 rounded-2xl p-3.5 flex items-center justify-between transition-all cursor-pointer shadow-xs group"
        >
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-white dark:bg-gray-800 text-[#190088] dark:text-indigo-400 flex items-center justify-center shadow-xs">
              <Clock className="w-4.5 h-4.5" />
            </div>
            <div>
              <p className="font-black text-gray-900 dark:text-gray-100">
                Programados & Catering: <span className="text-[#190088] dark:text-indigo-400">{todayScheduled.length} pedidos hoy</span>
              </p>
              <p className="text-[11px] text-gray-500 dark:text-gray-400">
                Anticipación y pase automático al Kanban antes del servicio.
              </p>
            </div>
          </div>
          <span className="text-[#190088] dark:text-indigo-400 text-xs font-black flex items-center gap-1 group-hover:translate-x-1 transition-transform">
            Ver Programados <ArrowRight className="w-3.5 h-3.5" />
          </span>
        </div>
      </div>
          ...
          {columns.map(col => {
            ...
            return (
              <div
                ...
                className={`w-[86vw] sm:w-80 md:w-88 flex-none snap-center bg-slate-50/80 dark:bg-gray-900/60 rounded-3xl border-2 p-3.5 flex flex-col justify-between shadow-xs space-y-3.5 transition-all duration-200 ${
                  dragOverCol === col.id
                    ? "border-[#FF3F1A] ring-4 ring-orange-500/20 bg-orange-50/50 dark:bg-orange-950/30 scale-[1.01]"
              ...
              >
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-gray-800">
                    <div className="flex items-center gap-2">
                      <div className={`w-7 h-7 rounded-xl flex items-center justify-center ${col.badgeBg}`}>
                        <Icon className="w-4 h-4" />
                      </div>
                      <div>
                        <h4 className="font-black text-xs text-gray-900 dark:text-gray-100">
                          {col.title}
                        </h4>
                      </div>
                    </div>

                    <span className={`font-mono font-black text-xs px-2.5 py-0.5 rounded-full ${col.badgeBg}`}>
                      {colOrders.length}
                    </span>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-1 max-h-[calc(100vh-340px)]">
                  {col.id === "NUEVO" && todayScheduled.length > 0 && (
                    <div className="p-3.5 rounded-2xl bg-indigo-50/90 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 space-y-2 text-xs shadow-xs">
                      <div className="flex items-center justify-between">
                        <span className="font-black text-[#190088] dark:text-indigo-300 flex items-center gap-1.5 text-[11px]">
                          <Clock className="w-3.5 h-3.5 text-[#FF3F1A]" /> Programado para Hoy:
                        </span>
                        <span className="font-mono font-black text-[10px] bg-white dark:bg-gray-800 px-2 py-0.5 rounded-md text-indigo-700 dark:text-indigo-300">
                      ...
                      <button
                        ...
                        className="w-full py-2 px-3 rounded-xl bg-[#190088] hover:bg-[#140070] text-white text-[11px] font-black flex items-center justify-center gap-1.5 shadow-xs cursor-pointer active:scale-95"
                      >
                  ...
                  {colOrders.length === 0 && (col.id !== "NUEVO" || todayScheduled.length === 0) ? (
                    <div className="p-8 text-center border-2 border-dashed border-slate-200 dark:border-gray-800 rounded-2xl text-gray-400 text-xs">
                      <p className="font-bold">Sin comandas en esta etapa</p>
                    </div>
                  ) : (
                    colOrders.map(order => {
                      ...
                      return (
                        <div
                          ...
                          className={`bg-white dark:bg-[#2C2D31] rounded-2xl border-2 p-4 shadow-xs hover:shadow-md transition-all cursor-grab active:cursor-grabbing space-y-3 ${
                            isDragging
                              ? "opacity-35 scale-95 border-dashed border-[#190088] ring-2 ring-indigo-500/30"
                              : isDelayed
                              ? "border-red-400 dark:border-red-800 bg-red-50/10"
                              : "border-slate-200/90 dark:border-gray-700 hover:border-[#190088]"
                          }`}
                        >
                          {/* Ticket Top Meta */}
                          <div className="flex items-center justify-between">
                          ...
                          <div className="bg-slate-50 dark:bg-gray-800/80 rounded-xl p-2.5 text-xs space-y-1 border border-slate-100 dark:border-gray-700">
                            ...
                            <div className="pt-1.5 border-t border-slate-200 dark:border-gray-700 flex justify-between font-black text-xs text-[#190088] dark:text-indigo-300">
                              <span>Total:</span>
                            ...
                            <div className="w-full bg-slate-100 dark:bg-gray-800 h-1.5 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full ${
                                  isDelayed ? "bg-red-500" : progressPercent > 80 ? "bg-amber-500" : "bg-emerald-500"
                                }`}
                          ...
                          >
                            {order.status === "NUEVO" && (
                              <>
                                ...
                                <button
                                  ...
                                  className="flex-1 py-1.5 px-3 rounded-xl bg-[#190088] hover:bg-[#140070] text-white text-[11px] font-black flex items-center justify-center gap-1 shadow-xs cursor-pointer active:scale-95"
                                >
                                  <span>Confirmar</span>
                            ...
                            {order.status === "CONFIRMADO" && (
                              <button
                                ...
                                className="w-full py-1.5 px-3 rounded-xl bg-[#FF3F1A] hover:bg-orange-600 text-white text-[11px] font-black flex items-center justify-center gap-1 shadow-xs cursor-pointer active:scale-95"
                              >
                            ...
                            {order.status === "EN_PREPARACION" && (
                              <button
                                ...
                                className="w-full py-1.5 px-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-black flex items-center justify-center gap-1 shadow-xs cursor-pointer active:scale-95"
                              >
                            ...
                            {order.status === "LISTO" && (
                              <button
                                ...
                                className="w-full py-1.5 px-3 rounded-xl bg-slate-900 hover:bg-black dark:bg-gray-700 text-white text-[11px] font-black flex items-center justify-center gap-1 shadow-xs cursor-pointer active:scale-95"
                              >
                            ...
                            {order.status === "FINALIZADO" && (
                              <div className="w-full text-center py-1 text-[10px] font-extrabold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950 rounded-xl">
                                Comanda Finalizada
                              </div>
                            )}
                          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filterOrdersList(orders).map(order => (
            <div
              key={order.id}
              onClick={() => setSelectedOrderId(order.id)}
              className="bg-white dark:bg-[#2C2D31] rounded-3xl border-2 border-slate-200/90 dark:border-gray-700 p-5 shadow-xs hover:border-[#190088] transition-all cursor-pointer space-y-3"
            >
              <div className="flex items-center justify-between">
                <span className="font-mono font-black text-xs text-gray-900 dark:text-gray-100">{order.id}</span>
                <span className="text-xs font-black text-[#190088] dark:text-indigo-300 font-mono">
      ...
      {/* Modal: Crear Nueva Comanda Manual */}
      {showManualModal && (
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-extrabold text-sm text-gray-900 dark:text-gray-100">
                    Nueva Comanda Manual (POS)
                  </h3>
                  <p className="text-xs text-gray-400">Ingreso rápido de comanda en salón o mostrador</p>
                </div>
              </div>
              <button
